...
const CourseScheduleViewer = () => {
  ...
  const sortByWeekday = (a: { date: string }, b: { date: string }) => {
    const daysOrder = [
      "Monday",
      "Tuesday",
      "Wednesday",
    ...
    const dayA = daysOrder.indexOf(
      dateA.toLocaleDateString("en-US", { weekday: "long" })
    );
    const dayB = daysOrder.indexOf(
      dateB.toLocaleDateString("en-US", { weekday: "long" })
    );

    return dayA - dayB;
  };

  const scheduleData: ScheduleData = rawLessons
    .reduce<ScheduleData>((acc, lesson) => {
